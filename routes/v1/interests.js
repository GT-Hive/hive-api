...
exports.getInterests = (req, res) => {
  Interest.findAll({ attributes }).then(interests => res.json({ interests }));
};

exports.getInterest = (req, res) => {
  const { id } = req.params;

  Interest.findOne({ where: { id }, attributes }).then(interest =>
    res.json({ interest })
  );
};
...
exports.createInterest = (req, res) => {
  ...
  Interest.create({ name: interest.name })
    ...
    .catch(err =>
      res.json({ error: 'Failed to create the interest due to an error' })
    );
};
...
exports.removeInterest = (req, res) => {
  ...
  Interest.destroy({ where: { id } })
    ...
    .catch(err =>
      res.json({ error: 'Failed to remove the interest due to an error' })
    );
};
...
exports.updateInterest = (req, res) => {
  ...
  Interest.update({ name: interest.name }, { where: { id } })
    .then(result => {
      if (result[0] > 0)
        return res.json({ success: 'Successfully updated the interest' });
      res
        .status(403)
        .json({
    ...
    .catch(err =>
      res.status(403).json({ error: 'Failed to update the interest' })
    );
};
