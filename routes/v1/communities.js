...
exports.getCommunities = (req, res) => {
  Community.findAll({ attributes, include: association.include }).then(
  ...
  db['Interest']
    ...
    .then(interest => {
      if (!interest)
        return res
          .status(404)
          .json({ error: 'Interest not found for community' });
    ...
    .catch(err =>
      res.status(404).json({ error: 'Interest not found for community' })
    );
};
...
exports.removeCommunity = (req, res) => {
  ...
  Community.destroy({ where: { id } })
    .then(removedCommunity => {
      if (!removedCommunity)
        return res.status(404).json({ error: 'Community is not found' });
      res.json({ success: 'Successfully removed the community' });
    })
    .catch(err =>
      res.json({ error: 'Failed to remove the community due to an error' })
    );
};
